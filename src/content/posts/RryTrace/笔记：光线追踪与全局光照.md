...
<p align="center">
  <img src="/markdown_picture/md_RayTracing/image.png" width="500">
</p>
<p align="center">
...
我们的目标是在一个球形光源对应的立体角范围内均匀采样方向。设当前着色点为 $\mathbf{P}$，球形光源球心为 $\mathbf{L}$，光源半径为 $R$，则两者距离为：
...
$$
distanceSquared=d^2=||\mathbf{L}-\mathbf{P}||^2
$$

观察点 $\mathbf{P}$ 看向球光源时，光源覆盖一个圆锥区域。设圆锥最大夹角为 $\theta_{max}$，根据球的几何关系：
...
\cos\theta_{max}
=
\sqrt{1-\frac{R^2}{distanceSquared}}
$$

为了在立体角内均匀采样，需要让：$z=\cos\theta$ 在线性范围：$[\cos\theta_{max},1]$ 内均匀分布，因此：
...
1+r_2
\left(
\sqrt{1-\frac{R^2}{distanceSquared}}-1
\right)
$$
...
其中：$\phi=2\pi r_1$，$\sin\theta=\sqrt{1-z^2}$
...
该方向位于以光源方向为中心、半角为 $\theta_{max}$ 的圆锥内部，实现了针对球形光源的均匀立体角采样，能够显著降低路径追踪中的方差。