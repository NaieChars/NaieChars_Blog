...
如果**一个文法**存在某个句子对应两棵或更多**不同的语法树（二义性看树，不看推导过程）**，则称该文法是**二义文法**。经典的例子是悬空 else 问题
