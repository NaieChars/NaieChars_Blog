---
title: CG 变换与软光栅化器核心算法的工程实现
published: 2026-07-08
pinned: true
description: 本节主要是针对顶点从局部空间到屏幕的各种变换，给出了 CG 中几个重要矩阵的数学推导，尤其是观察矩阵与透视投影矩阵，需要一定的线代基础。最后给出了软光栅化器核心算法的工程实现
...
[详细数学推导过程](https://learnopengl-cn.github.io/05%20Advanced%20Lighting/04%20Normal%20Mapping/)
...
$$
0\le k=\frac{dy}{dx}\le1
$$

因此每一步固定令 $x=x+1$，只需决定 $y$ 是否加 $1$，即在 **E**（下）和 **NE**（上）两个候选像素之间进行选择。
...
由于包含 $\frac12$，为了避免浮点运算，将其整体乘以 $2$：

$$
D=2d= 2dy(x + 1 - x_0) - 2dx(y - y_0) - dx
$$
...
$$
\overrightarrow{AB}\times\overrightarrow{AP},\quad
\overrightarrow{BC}\times\overrightarrow{BP},\quad
...
若三个叉积结果**同号**（全为正或全为负），说明点 $P$ 始终位于三条边的同一侧，因此 **$P$ 在三角形内部（或边界上）**；若结果有正有负，则点位于三角形外部。
